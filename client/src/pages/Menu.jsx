...
import { LuPlus, LuSearch, LuCheck, LuSoup, LuCookingPot, LuCroissant, LuCakeSlice, LuBeer, LuMenu } from "react-icons/lu"
...
export default function Menu(props) {
    ...
    return (
        <>
            <div className="flex flex-col items-center gap-8 h-full overflow-hidden">
                ...
                <main className="flex gap-16 w-[80%] mt-24 pt-16 pb-30 h-screen">
                    <div className="flex gap-6 w-full">
                        <div className="grid content-start gap-4 p-4 h-fit whitespace-nowrap rounded-sm bg-evergreen-darker">
                            <button onClick={() => setCurrentCategory("all")} className={`${currentCategory === "all" ? "bg-evergreen-light" : "bg-transparent"} flex items-center gap-2 rounded-sm text-left p-4 hover:bg-evergreen-light`}><LuMenu className="text-xl" /> ALL</button>
                            <button onClick={() => setCurrentCategory("mainDishes")} className={`${currentCategory === "mainDishes" ? "bg-evergreen-light" : "bg-transparent"} flex items-center gap-2 rounded-sm text-left p-4 hover:bg-evergreen-light`}><LuCookingPot className="text-xl" /> MAIN DISHES</button>
                            <button onClick={() => setCurrentCategory("soups")} className={`${currentCategory === "soups" ? "bg-evergreen-light" : "bg-transparent"} flex items-center gap-2 rounded-sm text-left p-4 hover:bg-evergreen-light`}><LuSoup className="text-xl" /> SOUPS</button>
                            <button onClick={() => setCurrentCategory("bakery")} className={`${currentCategory === "bakery" ? "bg-evergreen-light" : "bg-transparent"} flex items-center gap-2 rounded-sm text-left p-4 hover:bg-evergreen-light`}><LuCroissant className="text-xl" /> BAKERY</button>
                            <button onClick={() => setCurrentCategory("dessert")} className={`${currentCategory === "dessert" ? "bg-evergreen-light" : "bg-transparent"} flex items-center gap-2 rounded-sm text-left p-4 hover:bg-evergreen-light`}><LuCakeSlice className="text-xl" /> DESSERT</button>
                            <button onClick={() => setCurrentCategory("drinks")} className={`${currentCategory === "drinks" ? "bg-evergreen-light" : "bg-transparent"} flex items-center gap-2 rounded-sm text-left p-4 hover:bg-evergreen-light`}><LuBeer className="text-xl" /> DRINKS</button>
                        </div>
                        <div className="grid content-start w-full gap-6">
                            ...
                            <ul className="grid grid-cols-2 gap-6 pr-4 overflow-y-auto">{menuElements}</ul>
                        </div>
                    </div>
                </main>
            </div>
